...
Try<Isolator*> WindowsDiskIsolatorProcess::create(const Flags& flags)
{
  // TODO(jieyu): Check the availability of command 'du'.

  return new MesosIsolator(process::Owned<MesosIsolatorProcess>(
        new WindowsDiskIsolatorProcess(flags)));
}


WindowsDiskIsolatorProcess::Info::PathInfo::~PathInfo()
{
  usage.discard();
}


WindowsDiskIsolatorProcess::WindowsDiskIsolatorProcess(const Flags& _flags)
  : flags(_flags), collector(flags.container_disk_watch_interval) {}


WindowsDiskIsolatorProcess::~WindowsDiskIsolatorProcess() {}


Future<Nothing> WindowsDiskIsolatorProcess::recover(
...
Future<Option<ContainerLaunchInfo>> WindowsDiskIsolatorProcess::prepare(
...
Future<Nothing> WindowsDiskIsolatorProcess::isolate(
    const ContainerID& containerId,
    pid_t pid)
{
  if (!infos.contains(containerId)) {
...
Future<ContainerLimitation> WindowsDiskIsolatorProcess::watch(
    const ContainerID& containerId)
{
  if (!infos.contains(containerId)) {
    return Failure("Unknown container");
...
Future<Nothing> WindowsDiskIsolatorProcess::update(
    const ContainerID& containerId,
...
Future<Bytes> WindowsDiskIsolatorProcess::collect(
    const ContainerID& containerId,
    const string& path)
{
  ...
  return collector.usage(_path, excludes)
    .onAny(defer(
        PID<WindowsDiskIsolatorProcess>(this),
        &WindowsDiskIsolatorProcess::_collect,
        containerId,
        path,
        lambda::_1));
}
...
void WindowsDiskIsolatorProcess::_collect(
    const ContainerID& containerId,
    const string& path,
...
Future<ResourceStatistics> WindowsDiskIsolatorProcess::usage(
    const ContainerID& containerId)
{
  if (!infos.contains(containerId)) {
    return Failure("Unknown container");
...
Future<Nothing> WindowsDiskIsolatorProcess::cleanup(
    const ContainerID& containerId)
{
  if (!infos.contains(containerId)) {
