...
inline bool reparse_point_attribute_set(const std::string& absolute_path)
{
  ...
  const bool reparse_bit_set = (attributes & FILE_ATTRIBUTE_REPARSE_POINT) != 0;

  return reparse_bit_set;
}


// Attempts to extract symlink data out of a `REPARSE_DATA_BUFFER` (which could
// hold other things, e.g., mount point data).
inline Try<SymbolicLink> build_symbolic_link(
  const std::shared_ptr<REPARSE_DATA_BUFFER> data)
{
  ...
  const int target_name_start_index =
    data->SymbolicLinkReparseBuffer.SubstituteNameOffset / sizeof(WCHAR);
  const int target_name_length =
    data->SymbolicLinkReparseBuffer.SubstituteNameLength / sizeof(WCHAR);
  const WCHAR* target_name =
    data->SymbolicLinkReparseBuffer.PathBuffer + target_name_start_index;

  const int print_name_start_index =
    data->SymbolicLinkReparseBuffer.PrintNameOffset / sizeof(WCHAR);
  const int print_name_length =
    data->SymbolicLinkReparseBuffer.PrintNameLength / sizeof(WCHAR);
  const WCHAR* displayName =
    data->SymbolicLinkReparseBuffer.PathBuffer + print_name_start_index;

  struct SymbolicLink symlink;
  symlink.substitute_name.assign(target_name, target_name + target_name_length);
  symlink.print_name.assign(displayName, displayName + print_name_length);
...
inline Try<shared_handle> get_handle_no_follow(const std::string& absolute_path)
{
...
inline Try<SymbolicLink> get_symbolic_link_data(const shared_handle handle)
{
  ...
  std::shared_ptr<REPARSE_DATA_BUFFER> reparse_point_data(
      (REPARSE_DATA_BUFFER*)(new BYTE[reparse_point_data_size]));
  ...
  Try<SymbolicLink> symlink = build_symbolic_link(reparse_point_data);
  return symlink;
}
    ...
    const std::string& reparse_point)
{
  ...
  if (reparse_point_attribute_set(absolute_target_path)) {
    return Error(
